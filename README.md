...
# 编译
  ...
  cmake path/of/this/project
    (如果需要指定库位置，可使用：cmake 选项 -DCMAKE_PREFIX_PATH 
     e.g: cmake ../ -DCMAKE_PREFIX_PATH=your/path/of/boost_1_84_0-build/lib/cmake/boost_program_options-1.84.0/)
  cmake --build . -j*

# 功能
  JHS：解包、生成电子学配置文件、分析（这部分功能后被分开）
  script/draw_wave: 查看原初数据波形
  script/draw_time_dis: 按照时间分布重新划分事例,分析画图(主要功能)
  script/draw_time_dis1: (待重命名)拟合波形，生成事例按照时间和探测器通道划分的root文件[风末]
  script/track_seek： 画能谱、击中图，初步寻找3D track等[风末]
...
# 配置文件生成
    ...
    [config] 下：
      ...
      L<X0>C<X1>S<X2>=: 具体某通道压缩sigma倍数
          X0: 层号 X1: 通道号 X2: 压缩倍数值, 默认为<sigma-compres>对应值,若试将某通道设为0xFFF（上限）暂时解决方案为
          令X2为某极大值(e.g 10000)，计算后对应0x0FFF
      TT L<*>C<*>ADC<X>: =T 某通道触发阈值设置为X, L C意义同压缩阈值
      Rise Step Trigger Threshold: 波形上升幅度(为筛选有效波形)
      NHitChannel: 每层过阈通道数计数
      config: 配置文件生成位置

   写完配置文件后执行 ./JHS -c 生成配置文件（这些文件将被采数软件使用)
   注意删除历史文件(e.g rm -rf config && ./JHS -c && cp -r config /path/of/your/want)

#常见问题、错误解决
  o> 各类编译、链接错误：编译器版本过低、未安装依赖软件或不在环境中
  o> 解包事例数与存储事例数不同：数据包打包错误，暂时未解决
  o> 目标统计图中为空：检查波形是否合理、筛选条件(draw_time_dis)是否合理
