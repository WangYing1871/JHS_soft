...
struct lcs_t{
  ...
  ~lcs_t() noexcept=default;
  lcs_t() = default;
  lcs_t(int a, int b, float c):layer_id(a),channel_id(b), compres(c) {}

  bool operator==(lcs_t const& v) const{
    return layer_id==v.layer_id && channel_id==v.channel_id;
  }

  //bool operator<(lcs_t const& ) const{
  //  return (layer_id<<8+channel_id)<(rhs.layer_id<<8+rhs.channel_id)
  //};
...
struct lc_adc_t{
  ...
  uint16_t adc_t=0;
  lc_adc_t()=default;
  ~lc_adc_t() noexcept = default;
  lc_adc_t(uint16_t a, uint16_t b, uint16_t c):layer_id(a),channel_id(b),adc_t(c) {}
};
