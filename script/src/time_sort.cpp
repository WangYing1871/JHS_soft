  ...
  auto const& store = [&](){
    ...
    for (auto iter=tm_vs_adc.begin(); iter != ROFFSET(tm_vs_adc,1);){
      ...
      if (iter_next->first-iter->first>ww){
        if (false && //TODO
            std::distance(iter_front,iter_next)<=2){
        ...
        for(auto iter_tmp = iter_front; iter_tmp != iter_next; ++iter_tmp){
          ...
          if (adm_layer_id==0){
            if(v1_map.find(adm_channel_id)!=v1_map.end()){
              ...
              store_buf.chn_id[store_buf.nhits]=v1_map[adm_channel_id];
              std::copy(OFFSET(iter->second.adcs,0),ROFFSET(iter->second.adcs,0)
                  ,OFFSET(store_buf.adc_data[store_buf.nhits],0));
              store_buf.nhits++;
            }
            else if(v2_map.find(adm_channel_id)!=v2_map.end()){
              store_buf.chip_id[store_buf.nhits]= 2;
              store_buf.chn_id[store_buf.nhits]= v2_map[adm_channel_id];
              std::copy(OFFSET(iter->second.adcs,0),ROFFSET(iter->second.adcs,0)
                  ,OFFSET(store_buf.adc_data[store_buf.nhits],0));
              store_buf.nhits++;
            }
          ...
          if (adm_layer_id==2){
            if(v1_map.find(adm_channel_id)!=v1_map.end()){
              ...
              store1_buf.chn_id[store1_buf.nhits]= v1_map[adm_channel_id];
              std::copy(OFFSET(iter->second.adcs,0),ROFFSET(iter->second.adcs,0)
                  ,OFFSET(store1_buf.adc_data[store1_buf.nhits],0));
              store1_buf.nhits++;
            }
            else if(v2_map.find(adm_channel_id)!=v2_map.end()){
              store1_buf.chip_id[store1_buf.nhits]= 2;
              store1_buf.chn_id[store1_buf.nhits]= v2_map[adm_channel_id];
              std::copy(OFFSET(iter->second.adcs,0),ROFFSET(iter->second.adcs,0)
                  ,OFFSET(store1_buf.adc_data[store1_buf.nhits],0));
              store1_buf.nhits++;
            }
