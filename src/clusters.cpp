...
void hit_positon(long long idx
  ...
  TFolder* vf=nullptr;
  std::stringstream sstr(""); sstr<<"Event: "<<idx;
  if (view_file) vf = new TFolder(sstr.str().c_str(),sstr.str().c_str());
  ...
  auto const& get_char = [&](wave_t const& v){
    ...
    std::stringstream sstr_buf("");
    sstr_buf<<v.dim<<"-strip-"<<v.strip_id;
    auto* grp  = new TGraph();
    grp->SetName(sstr_buf.str().c_str());
    for (int i=0; i<1024; ++i)
      grp->SetPoint(i,i,v.wave[i]);
    if (view_file) {
      vf->Add(grp);
    }
  ...
  //auto pr = std::make_pair(0,1024);
  auto pr = std::make_pair(200,600);
