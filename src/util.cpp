...
namespace util{
std::vector<lcs_t> parse_lcs(std::string str){
  std::vector<lcs_t> ret;
  //std::unordered_set<lcs_t> ret;
  int index=0;
  float compres=0.;
  std::string str0(""), str1("");
  while(str.find_first_of('>')!=std::string::npos){
    std::string v_str = 
      str.substr(str.find_first_of('<')+1,str.find_first_of('>')-str.find_first_of('<')-1);
    if (index==0) str0=v_str;
    else if (index==1) str1=v_str;
    else if (index==2) compres=std::stof(v_str.c_str());
    str = str.substr(str.find_first_of('>')+1);
    index++; }
  std::istringstream isstr0(str0); std::istringstream isstr1(str1);
  for(;;){
    int a, b; isstr0>>a, isstr1>>b;
    if (isstr0.fail()) break;
    if (isstr1.fail()){
      throw std::invalid_argument("please check gramma of compres config");
    }
    ret.emplace_back(lcs_t{a,b,compres});
    //TODO
  }
  return ret;
}

std::vector<lc_adc_t> parse_lc_adc(std::string str){
  uint16_t adc=0xFFF;
  std::vector<lc_adc_t> ret;
  int index=0;
  std::string str0(""), str1("");
  while(str.find_first_of('>')!=std::string::npos){
    std::string v_str = 
    ...
    else if (index==2){ std::stringstream sstr(""); sstr<<v_str; sstr>>std::hex>>adc; }
    str = str.substr(str.find_first_of('>')+1);
    index++; }
  std::istringstream isstr0(str0); std::istringstream isstr1(str1);
  for(;;){
    uint16_t a, b; isstr0>>a, isstr1>>b;
    if (isstr0.fail()) break;
    if (isstr1.fail())
      throw std::invalid_argument("please check gramma of compres config");
    ret.emplace_back(lc_adc_t{a,b,adc});
    //TODO
  }
  return ret;
}
